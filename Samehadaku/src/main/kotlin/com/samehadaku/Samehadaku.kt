...
class Samehadaku : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        if (request.name == "Episode Terbaru") {
            ...
            val home = document.select("div.post-show ul li").mapNotNull { li ->
                ...
                val title = rawTitle.replace(Regex("Episode\\s*\\d+", RegexOption.IGNORE_CASE), "")
                    .removeBloat()
                    .trim()
            ...
            return newHomePageResponse(HomePageList(request.name, home, true), hasNext = home.isNotEmpty())
        }

        val document = app.get("$mainUrl/${request.data}$page").document
        val home = document.select("div.animposx").mapNotNull { it.toSearchResult() }
        return newHomePageResponse(HomePageList(request.name, home, false), hasNext = home.isNotEmpty())
    }
    ...
    private fun Element.toSearchResult(): AnimeSearchResponse? {
        ...
        return newAnimeSearchResponse(title, href, type) { posterUrl = poster }
    }

    override suspend fun search(query: String): List<SearchResponse> {
        return app.get("$mainUrl/?s=$query").document.select("div.animposx").mapNotNull { it.toSearchResult() }
    }
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val status = when (document.selectFirst("div.spe span:contains(Status)")?.ownText()) {
        ...
        val episodes = document.select("div.lstepsiode ul li").mapNotNull {
            ...
            val ep = Regex("Episode\\s?(\\d+)").find(a.text())?.groupValues?.getOrNull(1)?.toIntOrNull()
