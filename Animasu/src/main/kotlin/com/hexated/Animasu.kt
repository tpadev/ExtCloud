...
class Animasu : MainAPI() {
    ...
    private fun Element.toSearchResult(): AnimeSearchResponse {
        val href =
            getProperAnimeLink(fixUrlNull(this.selectFirst("a")?.attr("href")).toString())
        ...
        loadExtractor(url, referer, subtitleCallback) { link ->
            callback.invoke(
                ExtractorLink(
                    ...
                    link.url,
                    link.referer,
                    if (link.type == ExtractorLinkType.M3U8 || link.name == "Uservideo") link.quality else getIndexQuality(
                        quality
                    ),
                    link.type,
                    link.headers,
