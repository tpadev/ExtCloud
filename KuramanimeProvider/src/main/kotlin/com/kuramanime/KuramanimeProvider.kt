...
class KuramanimeProvider : MainAPI() {
    ...
    private suspend fun invokeLocalSource(
        ...
        if (server == "kuramadrive") {
            for (a in document.select("div#animeDownloadLink a")) {
                loadExtractor(a.attr("href"), "$mainUrl/", subtitleCallback, callback)
            }
        }
    }
    ...
    override suspend fun loadLinks(
        ...
        res.select("select#changeServer option").forEach { source ->
            ...
            if (server.contains(Regex("(?i)kuramadrive|archive"))) {
                ...
                app.get(link, referer = data, headers = headers, cookies = cookies).document
                    .select("div.iframe-container iframe").attr("src").let { videoUrl ->
