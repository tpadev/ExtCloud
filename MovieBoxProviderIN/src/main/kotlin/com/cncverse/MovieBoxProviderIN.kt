...
class MovieBoxProviderIN : MainAPI() {
    ...
    // Read secret keys from environment variables or project properties as fallback
    private val secretKeyDefault: String =
        System.getenv("MOVIEBOX_SECRET_DEFAULT")
            ?: (projectPropertyOrNull("MOVIEBOX_SECRET_DEFAULT") ?: "")

    private val secretKeyAlt: String =
        System.getenv("MOVIEBOX_SECRET_ALT")
            ?: (projectPropertyOrNull("MOVIEBOX_SECRET_ALT") ?: "")

    // Helper to safely read a Gradle project property at runtime (if present)
    private fun projectPropertyOrNull(name: String): String? {
        return try {
            val prop = System.getProperty(name)
            if (!prop.isNullOrBlank()) prop else null
        } catch (e: Exception) {
            null
        }
    }
    ...
    private fun getDefaultHeaders(xClientToken: String, xTrSignature: String, accept: String = "application/json", contentType: String = "application/json"): Map<String, String> {
        return mapOf(
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        if (response.code != 200) {
            throw ErrorLoadingException("Failed to load data: $responseBody")
        }
        val mapper = jacksonObjectMapper()
        val root = mapper.readTree(responseBody)
                        ...
                        if (responseBody != null) {
                            ...
                            if (streams != null && streams.isArray) {
                                for (stream in streams) {
                                    ...
                                    val linkHeaders = mutableMapOf("Referer" to mainUrl)
                                    if (signCookie != null) {
                                        linkHeaders["Cookie"] = signCookie
                                    }
