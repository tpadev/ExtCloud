...
class LayarKaca : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        return if (tvType == TvType.TvSeries) {
            val episodes = document.select("div.episode-list > a:matches(\\d+)")
                .mapNotNull { anchor ->
                    val href = fixUrl(anchor.attr("href"))
                    val episodeNumber = anchor.text().toIntOrNull()
                    val seasonNumber = anchor.attr("href")
                        .substringAfter("season-")
                        .substringBefore("-")
                        .toIntOrNull()

                    newEpisode(href) {
                        this.name = "Episode ${episodeNumber ?: anchor.text()}"
    ...
    override suspend fun loadLinks(
        ...
        val serverLinks = document.select("ul#loadProviders > li a").mapNotNull { element ->
            element.attr("abs:href").takeIf { it.isNotBlank() }
        }

        serverLinks.apmap { serverLink: String ->
            val embedDocument = app.get(serverLink, referer = data).document
            embedDocument.select("div.embed iframe, iframe").forEach { frame ->
                val src = listOf("data-src", "data-litespeed-src", "src")
                    .firstNotNullOfOrNull { key -> frame.absUrl(key).takeIf { it.isNotBlank() } }
                    ?: return@forEach
                val iframeUrl = src
                val referer = runCatching { getBaseUrl(iframeUrl) }.getOrDefault(serverLink) + "/"
